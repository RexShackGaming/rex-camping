...
local function placeProp(propmodel, item, campsitename, campsiteid)
    ...
    CreateThread(function()
        while not confirmed do
            ...
            if PromptHasHoldModeCompleted(SetPrompt) then
                ...
                DeleteObject(prop)
                if item == 'campflag' then
                    TriggerEvent('rex-camping:client:setupcampzone', propmodel, item, coords, heading)
                else
                    TriggerEvent('rex-camping:client:placecampsiteitem', propmodel, item, campsiteid, coords, heading)
                end
            end
...
RegisterNetEvent('rex-camping:client:createprop', function(data)
    placeProp(data.propmodel, data.item, data.campsitename, data.campsiteid)
end)
